...
## P. `/jobs` 作战台视觉强化
- [x] ~~复查当前 `/jobs` 桌面截图，确认第一屏仍偏普通白卡工作台~~
- [x] ~~将 `/jobs` PageHero 强化为深色任务头，并把四项统计合并为状态节奏条~~
- [x] ~~将岗位流改为带深色表头和列名的任务队列~~
- [x] ~~将右侧决策简报改为深色 Inspector，强化下一轮处理焦点~~
- [x] ~~按岗位状态为队列行增加语义化左侧状态色~~
- [x] ~~执行前端 lint / test / build、目标路由访问和桌面/移动截图复查~~
...
## T. 部署清理与生产复验
- [x] ~~确认仓库根目录仅保留 Job Tracker + JD Analyzer 代码与本项目文档资料~~
- [x] ~~回滚 Vercel 最新错误生产部署到最后正确部署~~
...
## U. 生产错误修复与域名优化
- [x] ~~复现生产 `/sources` 与 `/dashboard` 的 `TypeError: Failed to fetch`~~
- [x] ~~定位 Supabase REST 请求 `ERR_CONNECTION_CLOSED` 根因~~
