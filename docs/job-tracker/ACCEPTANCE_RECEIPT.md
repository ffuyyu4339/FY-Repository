...
| B-05 | Docker Compose 可启动基础服务 | BLOCKED | 当前 PowerShell 会话无法识别 `docker` 命令，未能启动联调 |
...
| E-02 | `/jobs/new` 页面可用 | PASS | 已接入新增岗位表单与 JD 解析入口，并通过同源 `/api` 代理兼容 Codespaces |
...
| F-10 | 支持人工修正分析结果 | PASS | 解析结果回填后可在表单中直接修正并保存，Codespaces 下由前端同源代理转发后端解析请求 |
...
| I-05 | Docker Compose 联调通过 | BLOCKED | 执行 `docker compose up --build` 时当前 PowerShell 会话无法识别 `docker` 命令 |
| I-06 | README 完整 | PASS | 已覆盖安装、本地开发、Docker、Linux 部署 |

---

## 验收证据
- Docker 启动命令：`docker compose up --build` 因当前会话无法识别 `docker` 命令而阻塞
- 前端 lint 输出：`npm run lint` 通过
- 后端 lint 输出：`ruff check .`、`black --check .` 通过
- pytest 输出：`python -m pytest tests/test_api.py -k analyzer -q` 通过
- 前端页面交互测试：`npm run test` 通过，包含 JobEditor 点击“解析 JD”自动回填验证
- 前端 build 输出：`npm run build` 通过
- Codespaces 请求修复：前端已改为同源 `/api` 请求，并由 Next.js 重写到 `BACKEND_INTERNAL_URL`，该变量已透传到 Docker 构建与运行环境
- 页面截图：待补充
...
|---|---|---|---|---|
| BUG-001 | 当前 PowerShell 会话无法识别 `docker` 命令，Docker Compose 联调与数据库实连验证被阻塞 | high | 是 | open |
| BUG-002 | 前端 build 类型错误已修复，问题已关闭 | low | 否 | closed |
| BUG-003 | Codespaces 下前端错误请求 `localhost:8000` 的问题已修复 | low | 否 | closed |
...
- 是否达到 MVP 发布条件：否
- 验收人：Codex / 你本人
- 验收时间：2026-04-16 00:48
- 最终说明：当前已完成前后端功能主链、前端 lint、前端 build、Codespaces 请求链路修复与前后端测试验证，尚需完成 Docker Compose 联调、数据库联通与手工验收。
